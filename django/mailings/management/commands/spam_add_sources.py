...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i in range(0, len(recipients), 5):
            ...
            send_mail(
                subject=subject,
                message=f"Здравствуйте!\n\nРанее вы прошли регистрацию на научную конференцию «{site}». Просим вас в срок до {deadline} указать используемые при подготовке доклада научные источники на сайте конференции, если вы этого ещё не сделали.\nУказать используемые источники можно на странице вашего доклада (раздел «Используемые источники»). Попасть на неё вы можете через свой личный кабинет.\n\n{footer}",
                html_message=render_to_string('conference/emails/mailings/spam/add_sources.html',
...
def get_recipients():
    articles = ArticleInfo.objects.filter(related_source__isnull=True).values('id')
